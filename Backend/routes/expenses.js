module.exports = (pool) => {
  ...
  router.get('/', checkAdminOrStaff, async (req, res) => {
    try {
      ...
      const productsResult = await pool.query('SELECT id, name FROM products WHERE library_id = $1', [req.libraryId]);
  ...
  router.post('/', checkAdminOrStaff, async (req, res) => {
    try {
      ...
      const result = await pool.query(
        'INSERT INTO expenses (title, amount, date, remark, branch_id, library_id) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *',
        [title, parseFloat(amount), date, remark, branch_id, req.libraryId]
      );
  ...
  router.put('/:id', checkAdminOrStaff, async (req, res) => {
    try {
      ...
      const result = await pool.query(
        'UPDATE expenses SET title = $1, amount = $2, date = $3, remark = $4, branch_id = $5 WHERE id = $6 AND library_id = $7 RETURNING *',
        [title, parseFloat(amount), date, remark, branch_id, parseInt(id, 10), req.libraryId]
      );
  ...
  router.delete('/:id', checkAdminOrStaff, async (req, res) => {
    try {
      ...
      const result = await pool.query('DELETE FROM expenses WHERE id = $1 AND library_id = $2 RETURNING *', [parseInt(id, 10), req.libraryId]);
      if (result.rows.length === 0) {
        return res.status(404).json({ message: 'Expense not found' });
      }
