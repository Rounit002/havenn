module.exports = (pool) => {
    ...
    try {
      ...
      let queryText = `
        SELECT s.id AS seat_id, s.seat_number,
               sch.id AS shift_id, sch.title AS shift_title, sch.description,
        ...
        WHERE s.library_id = $1 AND sch.library_id = $1
        AND (s.branch_id = sch.branch_id OR s.branch_id IS NULL OR sch.branch_id IS NULL)
      `;
  ...
  router.post('/', checkAdminOrStaff, async (req, res) => {
    try {
      ...
      const existingSeats = await pool.query(
        'SELECT seat_number FROM seats WHERE seat_number = ANY($1) AND ($2::integer IS NULL OR branch_id = $2) AND library_id = $3',
        [seatArray, branchIdNum, req.libraryId]
      );
      ...
      const insertQuery = 'INSERT INTO seats (seat_number, branch_id, library_id) VALUES ' +
        seatArray.map((_, i) => `($${i + 1}, $${seatArray.length + 1}, $${seatArray.length + 2})`).join(', ');

      await pool.query(insertQuery, [...seatArray, branchIdNum, req.libraryId]);
  ...
  router.delete('/:id', checkAdminOrStaff, async (req, res) => {
    try {
      ...
      await pool.query('DELETE FROM seat_assignments WHERE seat_id = $1 AND library_id = $2', [id, req.libraryId]);
      const result = await pool.query('DELETE FROM seats WHERE id = $1 AND library_id = $2 RETURNING *', [id, req.libraryId]);
  ...
  router.get('/:seatId/assignments', checkAdminOrStaff, async (req, res) => {
    try {
      ...
      const result = await pool.query(`
        ...
        WHERE sa.seat_id = $1 AND s.status = 'active' AND s.library_id = $2
