...
module.exports = (pool) => {
  ...
  router.post('/', checkPermissions(['manage_seats']), async (req, res) => {
    ...
    try {
      ...
      const result = await client.query(
        'INSERT INTO locker (locker_number, branch_id, is_assigned, library_id) VALUES ($1, $2, false, $3) RETURNING *',
        [locker_number, branch_id, req.libraryId] // Added branch_id to query
      );
  ...
  router.put('/:id', checkPermissions(['manage_seats']), async (req, res) => {
    ...
    try {
      ...
      const result = await client.query(
        'UPDATE locker SET locker_number = $1, branch_id = $2 WHERE id = $3 AND library_id = $4 RETURNING *',
        [locker_number, branch_id, id, req.libraryId]
      );
  ...
  router.delete('/:id', checkPermissions(['manage_seats']), async (req, res) => {
    ...
    try {
      await client.query('BEGIN');
      const id = parseInt(req.params.id, 10);
      const lockerCheck = await client.query('SELECT is_assigned, student_id FROM locker WHERE id = $1 AND library_id = $2', [id, req.libraryId]);
      ...
      if (lockerCheck.rows[0].is_assigned) {
        ...
        await client.query('UPDATE students SET locker_id = NULL WHERE id = $1 AND library_id = $2', [studentId, req.libraryId]);
      }
      const result = await client.query('DELETE FROM locker WHERE id = $1 AND library_id = $2 RETURNING *', [id, req.libraryId]);
      if (result.rows.length === 0) {
        await client.query('ROLLBACK');
        return res.status(404).json({ message: 'Locker not found' });
      }
