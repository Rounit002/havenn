...
const buildPgConfig = () => {
  ...
  if (!host || !user || !password || !database) {
    throw new Error(
      'DATABASE_URL is not defined and discrete DB config is incomplete. Provide DATABASE_URL or DB_HOST, DB_USER, DB_PASSWORD, DB_NAME (and optionally DB_PORT).'
    );
  }

  const sslEnabled = process.env.DB_SSL === 'true' || process.env.DB_SSL === '1' || process.env.NODE_ENV === 'production';
